...
<div class="min-h-screen font-sans selection:bg-[#1f71c1] selection:text-white bg-[#F8FAFC] text-slate-900">
	...
	<!-- FULL-WIDTH HERO SECTION (NBMS BRAND BLUE ACCENTS) -->
	...
	<main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-16">
		...
		{#if form?.success}
			<div class="p-8 sm:p-12 rounded-3xl border text-center space-y-6 animate-fade-in bg-white border-[#b9e0f6] shadow-2xl text-slate-900">
							...
							<div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
								{#each (productCms?.content?.servicesSection?.topBadges || [
									...
									<div class="group relative rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50/70 via-white to-white p-5 text-center shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center space-y-3">
										<div class="w-11 h-11 rounded-xl bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-[#1f71c1] group-hover:scale-110 group-hover:bg-[#1f71c1] group-hover:text-white transition-all">
							...
							<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
								{#if !contactCms?.content?.hidePhone && contactCms.content?.phone}
									<div class="p-4 rounded-2xl border bg-white border-slate-200 shadow-xs space-y-1">
										...
										<a href="tel:{contactCms.content.phone}" class="text-sm font-black text-[#1f71c1] hover:underline">{contactCms.content.phone}</a>
									</div>
								{/if}
								...
								{#if !contactCms?.content?.hideEmail && contactCms.content?.email}
									<div class="p-4 rounded-2xl border bg-white border-slate-200 shadow-xs space-y-1">
										...
										<a href="mailto:{contactCms.content.email}" class="text-sm font-black text-[#1f71c1] hover:underline">{contactCms.content.email}</a>
									</div>
								{/if}
